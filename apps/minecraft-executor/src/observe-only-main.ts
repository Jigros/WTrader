...
client.subscribe((event) => {
  ...
  if (event.type === 'GUI_OPENED' || event.type === 'GUI_UPDATED') process.stdout.write(`Observed GUI ${event.gui.id} type=${event.gui.windowType ?? 'unknown'} title=${event.gui.title} ${event.gui.signature}\n`);
});
